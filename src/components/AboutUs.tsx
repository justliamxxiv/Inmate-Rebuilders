...
export default function AboutUs() {
  ...
  const coreValues = [
    {
      title: "Inclusion",
      description: "We celebrate our differences. Every background and belief strengthens the fabric of our community.",
      icon: <Users className="w-8 h-8" />,
      color: "text-blue-500"
    },
    {
      title: "Compassion",
      description: "We believe in empathy. Everyone has a story, and listening matters most.",
      icon: <Heart className="w-8 h-8" />,
      color: "text-red-500"
    },
    {
      title: "Representation",
      description: "We fight to ensure everyone, regardless of financial background, has access to legal representation.",
      icon: <Scale className="w-8 h-8" />,
      color: "text-purple-500"
    },
    {
      title: "Empathy",
      description: "We meet individuals where they are, offering compassion and support to guide them towards a positive future.",
      icon: <Eye className="w-8 h-8" />,
      color: "text-green-500"
    },
    {
      title: "Restoration",
      description: "We hold firm: every inmate can be rehabilitated, reintegrated, and empowered to thrive in society once again.",
      icon: <RefreshCw className="w-8 h-8" />,
      color: "text-orange-500"
    },
    {
      title: "Human Dignity",
      description: "We stand resolute: honoring and respecting every individual, ensuring justice and dignity for all.",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {coreValues.map((value, index) => (
            <div 
              key={index} 
              className={`bg-white p-8 rounded-2xl border border-gray-100 hover:border-primary-200 transition-all duration-300 hover:shadow-lg group transition-all duration-700 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
              style={{ transitionDelay: `${200 + index * 100}ms` }}
            >
              {/* Icon Container */}
              <div className={`mb-6 p-4 rounded-xl bg-gray-50 inline-flex ${value.color} group-hover:bg-primary-50 transition-colors duration-300`}>
                {value.icon}
              </div>
